...
export interface RedactionContext {
  ...
  // Lazily-compiled single-pass alternation over all seeded ids (built once on
  // first redactString call, after seeding is complete). Null until built;
  // `false` means "built, but no id qualified" (skip the pass entirely — an
  // empty alternation would match at every position and corrupt output).
  _idRegex?: RegExp | false;
}
...
export function redactString(value: string, ctx: RedactionContext): string {
  ...
  // Seeded-id redaction in a SINGLE pass. The prior per-id split/join loop was
  // O(ids × content-bytes) — a real project with thousands of custom ids over a
  // large audit trail (or t242 test 17's 12k ids) blew the export/test budget.
  // One alternation regex, built once and cached on ctx, does it in one scan.
  // Longest-first ordering also fixes a latent bug: with overlapping ids
  // (build-auth before build-auth-extra) the old loop emitted <id:hash>-extra,
  // leaking the suffix; the regex prefers the longest match.
  const idRegex = getIdRegex(ctx);
  if (idRegex) {
    out = out.replace(idRegex, (m) => {
      const hash = ctx.idHashes.get(m);
      if (hash === undefined) return m; // not a seeded id (shouldn't happen)
...
// Build (once, then cache on ctx) a single alternation regex over every seeded
// id that qualifies (length >= 4). Ids are sorted LONGEST-first so an id that
// is a prefix of another does not win the match, and each is regex-escaped so a
// metachar in an id cannot over-match. Returns false when no id qualifies —
// callers must skip redaction entirely then (an empty alternation `//` matches
// at every position). Safe to cache because all ids are seeded before the first
// redactString call.
function getIdRegex(ctx: RedactionContext): RegExp | false {
  if (ctx._idRegex !== undefined) return ctx._idRegex;
  const ids = [...ctx.idHashes.keys()]
  ...
  ctx._idRegex = new RegExp(alternation, "g");
  return ctx._idRegex;
}
